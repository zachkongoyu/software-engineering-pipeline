# Plan Mode Prompt (QE / F3e)

A planning-only agent mode that researches and outlines multi-step plans
  ...
  "handoffs": [
    {
      ...
      "send": true
      // optionally uses chat.implementAgentModel for model override
    },
    {
...
- Additional tools via `chat.planAgentAdditionalTools` setting
- Model override via `chat.planAgent.defaultModel` setting
- `#tool:vscode/askQuestions` is always added to the tool set
